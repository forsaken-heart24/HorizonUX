...
if [[ "$TARGET_ADD_ROUNDED_CORNERS_TO_THE_PIP_WINDOWS" == "true" && "$BUILD_TARGET_ANDROID_VERSION" == "11" ]]; then
	...
	build_and_sign "${DECODEDAPKTOOLPATHS[3]}" "$HORIZON_FALLBACK_OVERLAY_PATH"
fi
...
if [ "$CUSTOM_WALLPAPER_RES_JSON_GENERATOR" == "true" ]; then
	...
	echo -e "  ]\n}" >> resources_info.json
	build_and_sign "${DECODEDAPKTOOLPATHS[4]}" "."
	echo -e "\e[0;31m######################################################################"
...
[[ "${BUILD_TARGET_SDK_VERSION}" == "35" && -n "${bring_experimental_features}" ]] && build_and_sign "${DECODEDAPKTOOLPATHS[5]}" $HORIZON_FALLBACK_OVERLAY_PATH
...
if ask "Do you want to add a stub app for missing activities?"; then
	...
	build_and_sign "${DECODEDAPKTOOLPATHS[7]}" "$SYSTEM_DIR/app/HorizonStub/"
fi
